...
const AddIrrigationMethod = () => {
  ...
  useEffect(() => {
    const fetchFarmers = async () => {
      try {
        const response = await fetch('/api/farmer/getAll');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchIrrigations = async () => {
      try {
        const response = await fetch('/api/irrigation/getAll');
        if (!response.ok) {
          throw new Error('Failed to fetch farmer data');
        }
        const data = await response.json();
  ...
  const handleAddIrrigationMethod = async () => {
    try {
      const response = await fetch('/api/irrigation/addNew', {
        method: 'POST',
        headers: {
  ...
  const handleAssignIrrigation = async () => {
    try {
      const response = await fetch(
          `/api/farmland/updateIrrigation/${selectedFarmland}/${selectedIrrigation}`,
          {
            method: 'PUT',
          }
      );
